...
- [ ] 8.3 Publish snapshots in order: clarity-protobuf 7.0-SNAPSHOT first, then clarity 5.0.0-SNAPSHOT — DEFERRED TO USER: outward-facing remote publish needing credentials. (Both already published to mavenLocal for local verification.)
- [ ] 8.4 Clear or bypass Gradle's changing-module cache and re-run `clarity-bench` `v5.0.0`; confirm no `NoClassDefFoundError` from a stale snapshot pairing — DEFERRED TO USER: release-time check, meaningful only after 8.3 publishes the relocated remote snapshot
- [ ] 8.5 Confirm `clarity-bench` `v3.1.3` and `v4.0.1` still run, verifying that pinned older versions remain isolated — DEFERRED TO USER: grouped with 8.4

## 9. Resolve open questions

- [ ] 9.1 Decide whether the descriptor byte-comparison becomes a permanent build guard or stays a one-off verification step — OPEN, user decision. Recommendation: make it a permanent guard (extraction logic already written), since it also catches accidental descriptor drift from future proto syncs — broader benefit than this change alone
